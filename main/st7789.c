/*
	Библиотека функций для обслуживания дисплея ST7789
*/
#include "hdr.h"
...
int lcdDrawString(TFT_t *dev, FontxFile *fx, uint16_t x, uint16_t y, uint8_t *ascii, uint16_t color)
{
	...
	if ((dev->_font_direction == 0) || (dev->_font_direction == 2)) return x;
	if ((dev->_font_direction == 1) || (dev->_font_direction == 3)) return y;
...
{
	if (dev->_bl >= 0) gpio_set_level(dev->_bl, 0 );
}
//-------------------------------------------------------------------------------------
// Backlight ON
void lcdBacklightOn(TFT_t *dev)
{
	if (dev->_bl >= 0) gpio_set_level( dev->_bl, 1 );
}
//-------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------
uint8_t fontWidth(FontxFile *fx)
{
    return (fx->w);
}
//-------------------------------------------------------------------------------------
uint8_t fontHeight(FontxFile *fx)
{
    return (fx->h);
}
//-----------------------------------------------------------------------------------------
char *mkLineCenter(char *str, uint8_t width, uint8_t *chars)
{
//char st[64] = {0x20};


    uint8_t slen = CONFIG_WIDTH / width;//16
    if (slen) {
    	if (chars != NULL) *chars = slen;
    	char *st = (char *)calloc(1, slen + 1);
    	if (st) {
    		memset(st, 0x20, slen);
...
//-----------------------------------------------------------------------------------------
char *mkLineWidth(char *str1, char *str2, uint8_t width)
{
char st[64] = {0};

    uint8_t slen = CONFIG_WIDTH / width;
